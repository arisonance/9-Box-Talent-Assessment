...
export default function Feedback360CreateModal({
  ...
  const handleCreateSurvey = async (saveAsDraft: boolean = false) => {
    ...
    try {
      ...
      // Create template (optional, for reuse)
      const { data: template, error: templateError } = await supabase
        .from('feedback_360_templates')
        .insert({
          organization_id: organizationId,
          template_name: `${surveyName} Template`,
          description: `Auto-generated template for ${surveyName}`
        })
        .select()
        .single();
      ...
      const templateQuestions = selectedQuestions.map((q, index) => ({
        template_id: template.id,
        question_template_id: q.template_id,
        custom_question_text: q.question_text,
        category: q.category,
      ...
      const { error: questionsError } = await supabase
        .from('feedback_360_template_questions')
        .insert(templateQuestions);

      if (questionsError) throw questionsError;

      // Update survey with template_id
      await supabase
        .from('feedback_360_surveys')
